...
/** @typedef {Readonly<{ready: true}> | Readonly<{applicationId: number, userVersion: 1 | 2 | 3 | 4, protocolVersion: 1, sqliteVersion: string, headRows: number, journalRows: number, maxGeneration: number}> | Readonly<{found: false}> | Readonly<{found: true, head: Readonly<{scope: AdminScope, generation: number, commitId: string, projectionFingerprint: string}>}> | Readonly<{verified: true}> | Readonly<{closed: true}>} AdminWorkerSuccessResult */
  ...
  if (type === "inspectSummary") {
    ...
    if (
      ...
      || (
        ...
        && result.userVersion !== 3 && result.userVersion !== 4
      )
