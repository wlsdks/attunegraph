...
| Persist a normalized current-head decision index with compact integer postings | `defer` | It is the strongest route to frontier-proportional reopen and traversal at million-scale, but it changes the physical schema, atomic commit set, corruption surface, and portable compatibility | Reconsider only after 1M/10M skewed-corpus evidence shows preparation or corpus-degree scans dominate and a versioned prototype preserves exact receipts while bounding page reads and write amplification |
