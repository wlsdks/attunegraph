...
| Persist a normalized current-head decision index | `measure-first (built storage candidate; package-private endpoint measured)` | Physical v3 atomically materializes exact current-head assertion, endpoint, temporal, provenance, and derivation atoms while retaining the v2 journal. A package-private bounded endpoint primitive now measures strict row reconstruction against full canonical decode, but no Engine/public query consumes it. A legal 32-assertion degree sweep shows normalized reconstruction wins on sparse endpoints and loses on dense endpoints, so an always-normalized path is rejected. | Before a public read path, prove exact-head/source-freshness admission, selected-bucket completeness, full Working Graph/receipt byte identity, and bounded sparse-to-dense fallback. Do not hard-code the observed crossover as a product threshold. Compact dictionaries remain deferred until scale/skew evidence attributes page cost. |
