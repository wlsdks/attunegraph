...
| Persist a normalized current-head decision index | `measure-first (built storage candidate)` | Physical v3 now atomically materializes exact current-head assertion, endpoint, temporal, provenance, and derivation atoms while retaining the v2 journal. Store open checks structural identity; explicit Admin and package-private per-scope verification strict-admit normalized rows. No query path consumes it, so it has not removed a query part or improved latency. | Add a read path only in a separate slice with use-before-read per-scope verification, bounded page/row counters, and byte-identical results/receipts. Compact integer dictionaries remain deferred until 100K/1M or skewed evidence attributes page cost. |
